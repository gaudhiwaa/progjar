...
class ProcessTheClient(threading.Thread):
    def __init__(self, connection, address, server):
        ...
        self.server = server  # Pass the server instance to access its methods
        ...
        while True:
            ...
            if data:
                ...
                if data.startswith(b'TIME') and data.endswith(b'\r\n'):
                    ...
                    self.server.update_response_count()  # Call the method to update the response count
            else:
                break
...
class TimeServer(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            client_thread = ProcessTheClient(client_socket, client_address, self)  # Pass 'self' as the 'server' argument
            client_thread.start()
